...
export default function Contact() {
  ...
  const contactMail = data?.contactMail;
  const socialLinks = data?.socialLinks?.[0];

  const copyEmail = () => {
    if (contactMail) navigator.clipboard.writeText(contactMail);
  };
          ...
          <div className="flex justify-center gap-4 mt-8">
            {socialLinks.Linkedin && (
              <a
                ...
                className="p-3 bg-card border border-border rounded-full text-muted-foreground hover:text-primary hover:border-primary/40 transition-colors"
              >
